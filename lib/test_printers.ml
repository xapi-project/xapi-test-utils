...
let option : 'a printer -> 'a option printer =
 fun pr x ->
  match x with None -> "None" | Some s -> pr s |> Printf.sprintf "Some %s"

(* Utility function to bracket a string *)
let bracket l r x = Printf.sprintf "%s%s%s" l x r

let pair : 'a printer -> 'b printer -> ('a * 'b) printer =
 fun pr_a pr_b (a, b) ->
  [pr_a a; pr_b b] |> String.concat ", " |> bracket "(" ")"

let tuple3 : 'a printer -> 'b printer -> 'c printer -> ('a * 'b * 'c) printer =
 fun pr_a pr_b pr_c (a, b, c) ->
...
let tuple4 :
    ...
    -> 'd printer
    -> ('a * 'b * 'c * 'd) printer =
 fun pr_a pr_b pr_c pr_d (a, b, c, d) ->
...
let tuple5 :
    ...
    -> 'e printer
    -> ('a * 'b * 'c * 'd * 'e) printer =
 fun pr_a pr_b pr_c pr_d pr_e (a, b, c, d, e) ->
  [pr_a a; pr_b b; pr_c c; pr_d d; pr_e e]
  |> String.concat ", "
  |> bracket "(" ")"
...
let array : 'a printer -> 'a array printer =
 fun pr x ->
  x |> Array.map pr |> Array.to_list |> String.concat "; " |> bracket "[|" "|]"

let list : 'a printer -> 'a list printer =
 fun pr x -> x |> List.map pr |> String.concat "; " |> bracket "[" "]"

(* Print an association list as "key: value; key: value" *)
let assoc_list : 'a printer -> 'b printer -> ('a * 'b) list printer =
 fun pr_a pr_b x -> x |> (assoc_pair pr_a pr_b |> list)
